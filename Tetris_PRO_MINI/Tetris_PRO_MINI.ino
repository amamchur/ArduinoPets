...
#define APIN_GAMEPAD_X_AXIS 1
#define APIN_GAMEPAD_Y_AXIS 0
#define DPIN_GAMEPAD_BUTTON 12
#define APIN_GAME_SPEED 2
...
const byte buttons[] PROGMEM = {DPIN_GAMEPAD_BUTTON};
...
void readGamepad() {
  float value = analogRead(APIN_GAMEPAD_X_AXIS);
  value = map(value, 0, 1023, 0, 6) - gamepadX;
  ...
  value = analogRead(APIN_GAMEPAD_Y_AXIS);
  value = map(value, 0, 1023, 0, 12) - gamepadY;
...
void calibrateGamepad() {
  for (int i = 0; i < 5; i++) {
    int value = analogRead(APIN_GAMEPAD_X_AXIS);
    gamepadX += map(value, 0, 1023, 0, 6);

    value = analogRead(APIN_GAMEPAD_Y_AXIS);
    gamepadY += map(value, 0, 1023, 0, 12);
  }
...
void readPotentiometer() {
  int value = analogRead(APIN_GAME_SPEED);
