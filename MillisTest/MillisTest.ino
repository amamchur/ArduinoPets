...
public:
  inline __attribute__((always_inline)) VolatileReader(const volatile T *ptr) : p(ptr) {
    prevSREG = SREG;
    cli();
  }
  ...
  inline __attribute__((always_inline)) ~VolatileReader() {
    SREG = prevSREG;
  }
private:
  uint8_t prevSREG;
...
int main() {
  ...
  uint32_t prev = 0;
  while (true) {
    uint32_t ms = VolatileReader<uint32_t>(&timer0_millis);
    if (ms - prev > 500) {
      BD13::toggle();
      prev = ms;
    }
